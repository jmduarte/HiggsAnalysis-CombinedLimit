...
for card in `ls dataCard_*txt`; do
    ...
    command="text2workspace.py -m 125.6 $card -P HiggsAnalysis.CombinedLimit.HiggsJPC_combo:twoHypothesisHiggs --PO=muFloating -o ${ID}.root -v 7; cd $ID; combine -M MultiDimFit ../${ID}.root --setPhysicsModelParameters cww_zz=0.5,r_ww=1"
